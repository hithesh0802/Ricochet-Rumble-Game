...
let shieldblue=false;
let shieldyellow=false;
let offsetX,offsetY;
const pieces = {
    tank1: tank1,
    titan1: titan1,
    ...
    const audioFull = document.getElementById('audioFull');
    audioFull.play();           
    setTimeout(() => {
        audioFull.pause();
    ...
    function startGame(mode) {
        ...
        if(isBotMode){
            document.getElementById('skipSpellforBlue').style.display='none';
            document.getElementById('skipSpellforYellow').style.display='none';
            document.getElementById('ShieldSpellforBlue').style.display='none';
            document.getElementById('ShieldSpellforYellow').style.display='none';
        }
    ...
    document.getElementById('ShieldSpellforBlue').addEventListener('click',()=>{
    ...
    document.getElementById('ShieldSpellforYellow').addEventListener('click',()=>{
    ...
    document.getElementById('skipSpellforBlue').addEventListener('click',()=>{
        if(currentPlayer==='white'){
        currentPlayer='black';
        changePlayer();
        playSpecialClip();
    ...
    document.getElementById('skipSpellforYellow').addEventListener('click',()=>{
        if(currentPlayer==='black'){
        currentPlayer='white';
        changePlayer();
        playSpecialClip();
    ...
    function playSpecialClip() {
        const audio = document.getElementById('specialEffects');
        audio.play();
        setTimeout(() => {
            audio.pause();
            audio.currentTime = 0; 
    ...
    function botMove() {
        ...
        if((selectedMove.start === 0 && selectedMove.end===7) || (selectedMove.start===0 && selectedMove.end)){
            playSpecialClip();
        }
...
function dragDrop(e){
    ...
    if((draggedElement.id === 'ricochet1' && (e.target.id ==='tank1'|| e.target.id==='tank2' || e.target.id==='semiRicochet1' || e.target.id ==='semiRicochet2' || e.target.id==='ricochet2')) || (draggedElement.id === 'ricochet2' && (e.target.id ==='tank1'|| e.target.id ==='tank2' || e.target.id==='semiRicochet1' || e.target.id ==='semiRicochet2' || e.target.id==='ricochet1'))){
        //swap ricochet with any other piece
        ...
        movePiece(Number(targetId),Number(startId),e.target.id);
        recordMove(Number(startId),Number(targetId));
        recordMove(Number(targetId),Number(startId));
        undoStack.push({ start: startId, end: targetId, piece: piece });
        undoStack.push({ start: targetId, end: startId, piece: e.target.id })
        // redoStack = [];  
        console.log('swapped');
        if(isBotMode){
            currentPlayer='black';
...
function pauseGame() {
    isPaused = true;
    let playgameMusic=document.getElementById('audioFull');
    playgameMusic.pause();
}
...
function redo() {
    ...
    if (childElement) {
        childElement.classList.remove('black', 'white');
        childElement.classList.add(playerGo );
    }
...
function nocollisionahead(id, direction) {
    ...
    let nextDiv = document.querySelector(selector);
    if (nextDiv && nextDiv.querySelector('.piece')) {
        divertHorizontal(nextDiv.getAttribute('square-id'),nextDiv.firstChild.id,direction);
        return false;
    }
    return true;
}
...
function nocollisionVerticalahead(id,bulldir,verdir){
    ...
    let nextDiv = document.querySelector(selector);
    if (nextDiv && nextDiv.querySelector('.piece')) {
        reflectedDivertHorizontal(nextDiv.getAttribute('square-id'),nextDiv.firstChild.id,verdir);
        return false;
    }
    return true;
}

function reflectedDivertHorizontal(id1,piece,verdir){
    if(piece=== 'titan1'){
        if(shieldblue){
        shieldblue=false;
        }else{
    ...
    appendAndRemoveDiv(); 
        ...
        if((piece==='semiRicochet1' && sr1direction=='center1' && diverter) || (piece==='semiRicochet2' && sr2direction=='center2' && diverter)  || (piece==='semiRicochet1' && sr1direction==='left' && !diverter) || (piece==='semiRicochet2' && sr2direction=='left' && !diverter)){
            ...
            appendAndRemoveDiv(); 
            }else if((((piece==='semiRicochet1' && sr1direction=='center2' && diverter) || (piece==='semiRicochet2' && sr2direction=='center1')) && diverter) || ((piece==='semiRicochet1' && sr1direction==='right' && !diverter) || (piece==='semiRicochet2' && sr2direction==='right')) && !diverter ) {
            let count = 0;
            let maxIterations = 7;
            let id=parseInt(id1,10);
            let emptyid;
...
function divertHorizontal(id1,piece,v1){
    if(piece=== 'titan1'){
        if(shieldblue){
        shieldblue=false;
        }else{  
...
function divertVertical(id1,piece,bulldir){
    if(piece ==='titan1'){
    ...
    }else if(piece==='tank1' || piece==='tank2'){
        if(bulldir==='left'){
            let count = 0;
            let maxIterations = 7;
            let id=parseInt(id1,10);
        ...
        function appendAndRemoveDiv() {
            emptyid= id+8;
            const intervalId = setInterval(() => {
                if (!nocollisionVerticalahead(id,'down','down')) {
                    clearInterval(intervalId);
                    return;
                }    
                    var selector = `div[square-id="${emptyid}"]`;
        ...
        function appendAndRemoveDiv() {
            ...
            const intervalId = setInterval(() => {
                if (!nocollisionVerticalahead(id,'up','up')) {
                    clearInterval(intervalId);
                    return;
                }    
                    var selector = `div[square-id="${emptyid}"]`;
...
function endGame(playerwon){
    ...
    pauseGame();
    let hi;
    if(playerwon==='White' || playerwon==='white' ){
        hi='Yellow'
    }else{
        if(!isBotMode)
        hi='Blue'
        else if(isBotMode){
            if(playerwon==='black' || playerwon==='Black')
            hi='Bot';
            else
            hi='Yellow';
        }        
